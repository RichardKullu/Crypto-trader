...
export const fetchCoins = async () => {
  try {
    const { data } = await axios
      .get(
        'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1&sparkline=true&price_change_percentage=1h%2C0.25h%2C0.5h%2C0.75h%2C1h%2C3h%2C6h%2C12h%2C24h%2C7d%2C14d%2C30d%2C90d'
      )
      .catch((error) => {
  ...
  const addCoin = (coin, qty, price) => {
    ...
    if (favCoin) {
      favCoin.map((crypto) => {
        if (crypto.coin === coin) {
          crypto.qty = (
            parseFloat(`${crypto.qty}`) + parseFloat(`${qty}`)
          ).toFixed(2);
          crypto.avg_price =
            (parseFloat(`${crypto.avg_price}`) + parseFloat(`${price}`)) / 2;
          found = true;
