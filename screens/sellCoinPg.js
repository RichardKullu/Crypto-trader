      ...
      <View style={styles.fixedForm}>
        <Formik
          ...
          onSubmit={(values, actions) => {
            setBalance(
              balance + parseFloat(values.quantity) * route.params.current_price
            );
            removeCoin(route.params.id, values.quantity);
            actions.resetForm();
            setPL(
              parseFloat(values.quantity) * route.params.current_price -
                parseFloat(values.quantity) * route.params.avg_price
            );
            showModal();
      ...
      <Modal visible={visible} onDismiss={hideModal}>
        <View style={styles.modalCard}>
          <Text style={styles.soldHeading}>
            You've successfully sold your cryptocurrency
          </Text>
          {Profit_Loss > 0 ? (
            <Text style={styles.soldProfit}>
              You've gained ${parseFloat(Profit_Loss).toFixed(2)}{' '}
            </Text>
          ) : (
            <Text style={styles.soldLoss}>
              You've lost ${parseFloat(Profit_Loss).toFixed(2)}{' '}
            </Text>
          )}
